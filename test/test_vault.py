...
class TestDefaultVault(TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.salt = b'\xe2\x98\xe5\xdc\xeb\xf5\xcc\xd8'
        cls.password = 'This is just a test'
        fd, fnm = tempfile.mkstemp(prefix='vault-')
        os.close(fd)
        os.unlink(fnm)
        cls.path = fnm
        DefaultVault.init(salt=cls.salt, key=cls.password, path=cls.path)
        vault = DefaultVault()
        vault['CHECK'] = 'CHECK ME'
    ...
    def test_vault_getitem(self):
        vault = DefaultVault()
        self.assertIn('CHECK', vault)
        self.assertEqual(vault['CHECK'], 'CHECK ME')

    def test_vault_setitem(self):
        vault = DefaultVault()
        self.assertNotIn('PASSWORD', vault)
        expected_value = 'Every g;;d boy does fine'
        vault['PASSWORD'] = expected_value
        self.assertIn('PASSWORD', vault)
        self.assertEqual(vault['PASSWORD'], expected_value)
